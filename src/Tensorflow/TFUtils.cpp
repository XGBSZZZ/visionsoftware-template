...
std::vector<std::string> TFUtils::GetAllOperationName()
{
    std::vector<std::string> op_name;
    size_t pos = 0;
    TF_Operation* oper;
    while ((oper = TF_GraphNextOperation(graph_def, &pos)) != nullptr) {
        op_name.push_back(TF_OperationName(oper));
    }
    return op_name;
}
...
std::vector<int64_t> TFUtils::GetOpsDimsByname(const std::string& name, int idx)
{
    auto output = GetOperationByName(name, idx);
    TF_Status* status = TF_NewStatus();
    int dimcount = TF_GraphGetTensorNumDims(graph_def,output, status);
    std::vector<int64_t> dims(dimcount,0);
    TF_GraphGetTensorShape(graph_def, output, dims.data(), dims.size(), status);
    return dims;
}
